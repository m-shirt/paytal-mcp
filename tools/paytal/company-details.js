...
const executeFunction = async (companyDetails) => {
  ...
  try {
   ...
   // console.error('Error creating company details:', error);
    return {
      error: `An error occurred while creating company details: ${error instanceof Error ? error.message : JSON.stringify(error)}`
    };
  }
};
  ...
  definition: {
    ...
    function: {
      ...
      parameters: {
        ...
        required: ['legal_name', 'commercial_name', 'type', 'registration_number', 'vat_number', 'currency', 'address', 'country_code', 'city', 'zip_code']
      }
    }
  }
};
