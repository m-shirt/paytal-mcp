...
export async function setupServerHandlers(server, tools, prompts) {
  ...
  server.setRequestHandler(ListPromptsRequestSchema, async () => ({ prompts: transformedPrompts }));
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    for (const p of required) if (!(p in args)) throw new McpError(ErrorCode.InvalidParams, `Missing required parameter: ${p}`);
  ...
  server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    if (!Array.isArray(result.messages)) throw new McpError(ErrorCode.InternalError, 'Prompt handler must return messages array');
    return result;
  });
}
...
export async function runCLI() {
  ...
  if (args.includes('--streamable-http')) {
    ...
    expressApp.listen(port, () => console.log(`Streamable HTTP running on port ${port}`));
    ...
    expressApp.get('/sse', async (req, res) => {
      ...
      res.on('close', async () => { delete localTransports[transport.sessionId]; await server.close(); delete localServers[transport.sessionId]; });
      await server.connect(transport);
    });
    ...
    const port = process.env.PORT || 3001;
    expressApp.listen(port, () => console.log(`SSE server running on port ${port}`));
  } else {
    const tools = await discoverTools();
    const prompts = await discoverPrompts();
    const server = new Server({ name: SERVER_NAME, version: '0.1.0' }, { capabilities: { tools: {}, prompts: {} } });
